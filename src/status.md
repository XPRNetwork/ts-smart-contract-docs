...
| WebAssembly spec                  | Engines                             | Proton (flag)   | Perspective
|-----------------------------------|-------------------------------------|-------------------------|------------------------------------
| ✔️ **Finished proposal**
...
| <Badge text="uncertain" type="warning"/> | Proton is uncertain about this specification and not in a hurry to implement it
| <Badge text="harmful" type="error"/> | Proton considers this specification to be harmful in its current state
...
| JSON                                              | JSON is not strictly typed in nature, so we haven't settled on a standard yet. Solutions developed by the community: [proton-json](https://github.com/nearprotocol/proton-json)
| RegExp                                            | Regular expressions need quite a lot of supporting code with many quirks, and we haven't decided on an implementation yet. Solutions developed by the community: [proton-regex](https://github.com/ColinEberhardt/proton-regex)
| Date                                              | There is initial support for the UTC parts, but WebAssembly lacks access to the system's time zone data. In general the Temporal proposal could be more feasible to adopt. Solutions developed by the community: [proton-temporal](https://github.com/ColinEberhardt/proton-temporal).
||
| 🥚&nbsp;**Not&nbsp;implemented**
...
| BigInt                                            | Instead of BigInts, Proton has opted to utilize WebAssembly's 64-bit integers. It is currently unclear how both concepts could mix.
||
| 🕳️&nbsp;**Not&nbsp;supported**
| Dynamicness                                       | Proton avoids overly dynamic JavaScript features by design and focuses on static compilation.
...
Proton intentionally avoids very dynamic JavaScript features that cannot be compiled efficiently, like for example:
...
||
| 🐣&nbsp;**Limited**
| Linting                 | Proton re-uses TypeScript tooling with a `// @ts-ignore` here or there but would benefit from a dedicated language server eventually. Solutions developed by the community: [asls](https://github.com/Shopify/asls)
