...
In Proton, host functionality can be imported by utilizing the ambient context, that is using a `declare` statement:
...
Proton has experimental support for WASI, which can be targeted by adding `import "wasi"` to the entry file. Doing so will install WASI-aware implementations of `abort`, `trace` and `seed` for example, but will require a polyfill on the Web.
...
Decorators work more like compiler annotations in Proton and are evaluated at compile time.
