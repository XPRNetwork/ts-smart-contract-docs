...
Similar to TypeScript's `tsc` transpiling to JavaScript, Proton's `asc` compiles to WebAssembly.
...
Specifying the base offsets of compiler-generated memory respectively the table leaves some space for other data in front. In its current form this is mostly useful to link additional data into an Proton binary after compilation, be it by populating the binary itself or initializing the data externally upon initialization. One good example is leaving some scratch space for a frame buffer.
...
<script type="importmap">
{
  "imports": {
    ...
    "proton": "https://cdn.jsdelivr.net/npm/proton@x.x.x/dist/proton.js",
    "proton/asc": "https://cdn.jsdelivr.net/npm/proton@x.x.x/dist/asc.js"
  }
}
</script>
<script type="module">
...
Note that the matching versions of the respective dependencies need to be filled in instead of `x.x.x`. Current versions can be obtained from the generated [web.html](https://cdn.jsdelivr.net/npm/proton/dist/web.html) file. The [es-module-shims](https://github.com/guybedford/es-module-shims) dependency polyfills support for import maps where not yet available.
...
* JavaScript globals in `globalThis` can be accessed directly via the `env` module namespace. For example, `console.log` can be manually imported through:
  ...
  ```
  Note that this is just an example and `console.log` is already provided by the standard library when called from an Proton file. Other global functions not already provided by the standard library may require an import as of this example, though.
...
Proton is compiled statically, so code transformation cannot be done at runtime but must instead be performed at compile-time. To enable this, the compiler frontend \(asc\) provides a mechanism to hook into the compilation process before, while and after the module is being compiled.
...
import { Transform } from "proton/transform"
...
With Proton being very similar to TypeScript, there comes the opportunity to compile the same code to JavaScript with `tsc` and WebAssembly with `asc`. The Proton compiler itself is portable code. Writing portable code is largely a matter of double-checking that the intent translates to the same outcome in both the strictly typed Proton and the types-stripped-away TypeScript worlds.

### Portable standard library

Besides the full standard library, Proton provides a portable variant of the functionality that is present in both JavaScript and WebAssembly. In addition to that, the portable library lifts some of the functionality that is only available with `asc` to JavaScript, like the portable conversions mentioned below.
...
To use the portable library, extend `proton/std/portable.json` instead of `proton/std/assembly.json` within `tsconfig.json` and add the following somewhere along your build step so the portable features are present in the environment:

```js
import "proton/std/portable.js"
```

Note that the portable standard library is still a work in progress and so far focuses on functionality useful to make the compiler itself portable, so if you need something specific, feel free to improve [its definitions and feature set](https://github.com/Proton/proton/tree/main/std/portable).
